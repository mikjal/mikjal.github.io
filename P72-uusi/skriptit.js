...
let kartta = new L.map('map', {
    center: [61.878559282686375, 29.094800949096683],
    zoom: 17,
    rotate: true,
    rotateControl: {
...
function locateButton(paalle) {
    if (paalle) {
        ...
        locatePaalla = false;
        if (omaPaikka) omaPaikka.removeFrom(kartta);
        omaPaikka = null;
...
function paivitaOmaPaikka(latlng) {
    if (omaPaikka) {
        ...
        if (seuraavaPiste <= paikat.length-1) {
            ...
            // tämä pois lopullisesta versiosta
            //kartta.panTo(markerit[seuraavaPiste].getLatLng());

            let etaisyys = kartta.distance(latlng,markerit[seuraavaPiste].getLatLng());
            let etastr;
            // etäisyys seuraavaan karttamerkkiin
            // yli 1000 metriä, näytetään kilometreinä
            if (etaisyys >= 1000) {
                //etaisyys /= 1000;
                etastr = (etaisyys / 1000).toString();
                etastr = etastr.slice(0,etastr.indexOf('.')+2);
                etastr += ' km';
            } else { // alle 1000 metriä, näytetään metreinä
